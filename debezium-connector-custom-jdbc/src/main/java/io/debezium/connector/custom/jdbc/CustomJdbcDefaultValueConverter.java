...
// TODO: Important - reimplement this for CustomJdbc
public class CustomJdbcDefaultValueConverter implements DefaultValueConverter {
                ...
                switch (jdbcType) {
                    case Types.DATE:
                        return JdbcConnection.querySingleValue(
                                ...
                                "SELECT DATE(" + value + ") FROM SYS.DUMMY",
                                st -> {
                                },
                                rs -> rs.getDate(1));
                    case Types.TIME:
                        return JdbcConnection.querySingleValue(
                                connection.connection(),
                                "SELECT TIME(" + value + ") FROM SYS.DUMMY",
                                st -> {
                                },
                                rs -> rs.getTime(1));
                    case Types.TIMESTAMP:
                        return JdbcConnection.querySingleValue(
                                connection.connection(),
                                "SELECT TIMESTAMP(" + value + ") FROM SYS.DUMMY",
                                st -> {
                                },
